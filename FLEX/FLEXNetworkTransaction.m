...
- (NSData *)cachedRequestBody {
    if (!_cachedRequestBody) {
        ...
        if (self.request.HTTPBody != nil) {
            ...
            #define kFLEXRequestBodyBufferSize 1024
            uint8_t buffer[kFLEXRequestBodyBufferSize];
            NSMutableData *data = [NSMutableData new];
            [bodyStream open];
            NSInteger readBytes = 0;
            do {
                readBytes = [bodyStream read:buffer maxLength:kFLEXRequestBodyBufferSize];
                [data appendBytes:buffer length:readBytes];
            } while (readBytes > 0);
            [bodyStream close];
            _cachedRequestBody = data;
            #undef kFLEXRequestBodyBufferSize
        }
    }
    return _cachedRequestBody;
}
